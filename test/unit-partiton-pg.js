...
describe('PartionPg Unit Tests', function () {
    ...
    it('should execute correct sql when defining a table', async function () {
        ...
        dsql TEXT;
        BEGIN
        dsql:= 'CREATE TABLE IF NOT EXISTS "Anukram".'|| quote_ident(table_name) || '( "time" bigint, "tagid" integer, "value" double precision, "quality" integer ,CONSTRAINT '|| quote_ident(primarykey_name)||' PRIMARY KEY ("time","tagid"))';
        EXECUTE dsql;
        dsql:= 'CREATE INDEX IF NOT EXISTS '|| quote_ident(index_name) ||' ON "Anukram".' || quote_ident(table_name) || ' ( "time" DESC, "quality" ASC)';
        EXECUTE dsql;
        END$$;`;
